...
class Parser:
    ...
    def parse_top(self):
        ...
        parser.add_argument('--model_type', type=str, choices=['pop', 'nmf', 'dmf', 'spop', 'sasrec', 'draftrec'], help='Selects the model for the experiment')
        args = parser.parse_known_args(self.sys_argv)[0]
        return vars(args)

    def parse_dataloader(self):
        parser = argparse.ArgumentParser(allow_abbrev=False)
        parser.add_argument('--train_dataloader_type', type=str, choices=['interaction', 'sequential', 'match'], help='Selects the trainer for the experiment')
