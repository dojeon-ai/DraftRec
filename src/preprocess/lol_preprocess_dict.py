...
def create_interaction_dataframe(match_data, user_dict,categorical_ids):
    num_participants = 10
...
if __name__=='__main__':
    ...
    parser.add_argument('--data_dir', type=str, default='/home/dongyoonhwang/draftrec/preprocess/lol_data') 
    ...
    with open(args.data_dir + '/lol_categorical_ids.pickle', 'rb') as f:
        categorical_ids = pickle.load(f)

    with open(args.data_dir + '/lol_match_dataframe.pickle', 'rb') as f:
    ...
    new_match_dataframe = create_match_dataframe(new_match_dict)

    # normalize stats
    new_user_history_dict = normalize_stats(new_user_history_dict, args)
    ...
    interaction_data = {}
    interaction_data['train'] = create_interaction_dataframe(new_match_dataframe['train'], new_user_history_dict,categorical_ids)
    interaction_data['val'] = create_interaction_dataframe(new_match_dataframe['val'], new_user_history_dict,categorical_ids)
    interaction_data['test'] = create_interaction_dataframe(new_match_dataframe['test'], new_user_history_dict,categorical_ids)

    print('Finish creating interaction data')

    with open('./lol_user_history_data.pickle', 'wb') as f:
        pickle.dump(new_user_history_dict, f, pickle.HIGHEST_PROTOCOL)
    with open('./lol_new_match_dict.pickle', 'wb') as f:
        pickle.dump(new_match_dict, f, pickle.HIGHEST_PROTOCOL)
    with open('./lol_new_match_dataframe.pickle', 'wb') as f:
        pickle.dump(new_match_dataframe, f, pickle.HIGHEST_PROTOCOL)
    with open('./lol_interaction_data.pickle', 'wb') as f:
        pickle.dump(interaction_data, f, pickle.HIGHEST_PROTOCOL)
